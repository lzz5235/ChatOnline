...
struct UserInformation
{
        ...
        {
                qos <<pi.userID << pi.account << pi.password << pi.nickName << pi.avatarNumber
                        << pi.status << pi.mobileNumber
                        << pi.phoneNumber << pi.birthday<< pi.city << pi.aboutMyself
                        << pi.mail << pi.dormitory;
                return qos;
        }
        friend QDataStream & operator>> (QDataStream &qis, UserInformation &pi)
        {
                qis >> pi.userID >> pi.account >> pi.password >> pi.nickName >> pi.avatarNumber
                        >> pi.status >> pi.mobileNumber
                        >> pi.phoneNumber >> pi.birthday >> pi.city >> pi.aboutMyself
                        >> pi.mail >> pi.dormitory;
                return qis;
...
struct LoginInformation
{
        ...
        {
                qos << userID << li.account << li.password << li.status;
                return qos;
        }
        friend QDataStream & operator>> (QDataStream &qis, LoginInformation &li)
        {
                qis >> li.userID >> li.account >> li.password >> li.status;
                return qis;
        }
};
